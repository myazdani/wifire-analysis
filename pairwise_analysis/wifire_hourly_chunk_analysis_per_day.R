...
res = as.data.frame(dt[,lapply(.SD, FUN = function(x) as.double(mean(x))), by = list(day, hour.chunk), .SDcols = 2:693])
rm(dt)
...
ggplot(mds.S, aes(x = coord.1, y = coord.2, label = hour.chunk)) + facet_wrap( ~ day, nrow = 2) + geom_text() + ggtitle("MDS of Saturation Distributions")
mds.V = return_mds_per_day(res.V)
ggplot(mds.V, aes(x = coord.1, y = coord.2, label = hour.chunk)) + facet_wrap( ~ day, nrow = 2) + geom_text() + ggtitle("MDS of Value Distributions")
